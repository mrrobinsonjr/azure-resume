---
title: "Cloud Architect"
company: "USCENTCOM (supported)"
location: "MacDill AFB, Tampa, FL"
start: "2022"
end: "2023"
role_type: "Contractor"
tags:
  - "cloud"
  - "architecture"
  - "devsecops"
summary: "Led secure cloud architecture and the initial transition to Azure Government IL5/IL6 for USCENTCOM, delivering IaC frameworks, DevSecOps platforms, and data analytics foundations aligned to DoD governance."
highlights:
  - "Architected and delivered the first Azure Gov IL5/IL6 deployments for the command."
  - "Built reusable Terraform/ARM modules and standardized provisioning to reduce drift and provisioning time."

outputs:
  - "HTML"
tech:
  - "Azure Government IL5/IL6"
  - "Terraform"
  - "ARM"
  - "GitHub"
  - "Azure DevOps"
  - "Azure Synapse"
---

Mission / Scope

Lead Cloud Architect responsible for USCENTCOMâ€™s initial adoption of Azure Government IL5/IL6 environments. Focused on secure cloud design, DevSecOps enablement, IaC frameworks, and data analytics architecture to meet RMF and SRG requirements.

What I Owned

- Secure enterprise cloud architecture and boundary configurations for IL5/IL6 workloads.
- Design and implementation of IaC modules (Terraform/ARM) for standardized, repeatable deployments.
- DevSecOps ecosystem architecture and automation patterns across GitHub and Azure DevOps.
- Data lakehouse design using Azure Synapse and medallion patterns to enable analytics and operational reporting.

Impact Highlights

- Reduced provisioning time and configuration drift through reusable IaC frameworks.
- Accelerated secure deployment pipelines and established a DevSecOps culture across engineering teams.

Tooling / Platforms

- Azure Government (IL5/IL6), Azure Synapse
- Terraform, ARM
- GitHub, Azure DevOps
- Networking: VNet design, identity integrations

Selected Deliverables

- Terraform/ARM IaC modules and provisioning templates.
- Architecture diagrams and engineering trade studies for IL5/IL6 boundary and segmentation.

