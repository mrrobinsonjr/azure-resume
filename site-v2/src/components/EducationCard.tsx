...
function EducationCard({ education, index = 0 }: EducationCardProps) {
  ...
  return (
    <article className="group relative pl-8" data-role-card={education.id}>
      {/* Timeline rail */}
      <span className="absolute left-0 top-1.5 h-full w-px bg-slate-200 transition-colors group-hover:bg-brand-300/60" aria-hidden="true" />

      {/* Node dot — book icon instead of role dot */}
      <span className="absolute left-[3px] top-1.5 z-10 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-white ring-4 ring-slate-200 group-hover:ring-brand-200">
        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 text-brand-600" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
          ...
          <path d="M22 3h-9a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2Z" />
        </svg>
      </span>

      {/* Card body */}
      <div className="relative -ml-[1px] rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-[1px] hover:shadow-lg hover:border-brand-200/70">
        {/* School */}
